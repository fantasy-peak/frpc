...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(BankInfo,
                                   name,
                                   type,
                                   test_one,
                                   test_two,
...
inline std::string toString(const BankInfo& value) {
    ...
    ss << "BankInfo{" << value.name << "," << toString(value.type) << ","
       << value.test_one << "," << value.test_two << ","
       << toString(value.test_map_one) << "," << toString(value.test_map) << ","
       << toString(value.test_vector) << "," << toString(value.info) << ","
...
inline bool operator==(const BankInfo& lhs, const BankInfo& rhs) {
    return (lhs.name == rhs.name) && (lhs.type == rhs.type) &&
           (lhs.test_one == rhs.test_one) && (lhs.test_two == rhs.test_two) &&
           (lhs.test_map_one == rhs.test_map_one) &&
           (lhs.test_map == rhs.test_map) &&
           (lhs.test_vector == rhs.test_vector) && (lhs.info == rhs.info) &&
